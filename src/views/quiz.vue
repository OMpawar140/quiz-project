...
// Fetch quiz data from the server based on quizId
...
const submitQuiz = async () => {
  try {
    ...
    const quizId = quiz.value._id; // Assuming `quiz` contains the quiz object
    const responses = selectedAnswers.value; // Assuming `selectedAnswers` is an array containing selected answer indices
    let score = 0;
    // responses.forEach(async response => {
    //   const question = quiz.value.questions[response.questionIndex];
    //   // Check if the question exists before accessing its properties
    //   if (response.selectedAnswerIndex === question.correctAnswerIndex) {
    //     score += quiz.value.marksPerQuestion;
    //   }
    // });
    const marks = quiz.value.marksPerQuestion; 

    // Send POST request to server with quiz responses data
...
//   try {
//     const response = await axios.get(`http://localhost:3000/quiz-responses/${quizId}`);
//     quizResponses.value = response.data;
//   } catch (err) {
